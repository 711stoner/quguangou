...
async function renderTotalDownloadCount() {
  ...
  try {
    const response = await fetch("https://api.github.com/repos/711stoner/quguangou/releases/tags/v0.2.11");
    if (!response.ok) throw new Error("download count unavailable");
    const release = await response.json();
    const asset = release.assets.find((item) => item.name === "quguangou-chrome-v0.2.11.zip");
